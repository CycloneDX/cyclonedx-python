...
class PdmParser(BaseParser):

    def __init__(
            self, pdm_lock_contents: str, use_purl_bom_ref: bool = False,
            *,
        ...
        debug_message('loading pdm_lock_contents')
        pdm_lock = load_toml(pdm_lock_contents)

        debug_message('processing pdm_lock')
        for package in pdm_lock['package']:
...
class PdmFileParser(PdmParser):

    def __init__(
            self, pdm_lock_filename: str, use_purl_bom_ref: bool = False,
            *,
            debug_message: DebugMessageCallback = quiet
    ) -> None:
        debug_message('open file: {}', pdm_lock_filename)
        with open(pdm_lock_filename) as plf:
            super(PdmFileParser, self).__init__(
                pdm_lock_contents=plf.read(), use_purl_bom_ref=use_purl_bom_ref,
